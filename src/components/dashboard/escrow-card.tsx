...
export function EscrowCard({
  ...
  const isOpenJob =
    !escrow.beneficiary ||
    escrow.beneficiary === "" ||
    escrow.beneficiary === "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF";
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "Dispute Resolved": return "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300";
  ...
  const progressPercentage =
    escrow.totalAmount !== "0"
      ? (Number.parseFloat(escrow.releasedAmount) / Number.parseFloat(escrow.totalAmount)) * 100
      : 0;
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              {escrow.projectTitle && escrow.projectDescription && (
                <p className="text-sm text-muted-foreground mb-2">{escrow.projectDescription}</p>
              )}
              <div className="flex items-center gap-4 text-sm text-gray-600">
                <div className="flex items-center gap-1">
                  <Clock className="h-4 w-4" />
                  <span>{Math.round(escrow.duration / (24 * 60 * 60))} days</span>
                </div>
                <div className="flex items-center gap-1">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {expandedEscrow === escrow.id ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
              </Button>
            </div>
          </div>
        </CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <div className="font-semibold">{fmtXlm(escrow.totalAmount)}</div>
              </div>
              <div>
                <span className="text-gray-600">Released:</span>
                <div className="font-semibold">{fmtXlm(escrow.releasedAmount)}</div>
              </div>
              <div>
                ...
                <div className="font-semibold flex items-center gap-1">
                  ...
                  {(() => {
                    ...
                    return <span className={message.color}>{message.text}</span>;
                  })()}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4 pt-4 border-t">
                  ...
                  {escrow.milestones.map((milestone, idx) => (
                    ...
                    >
                        ...
                        <div className="p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <EvidenceSubmissionButton
                              ...
                              milestoneIndex={idx}
                              onEvidenceSubmitted={() => {
                                toast({ title: "Evidence submitted", description: "Your evidence has been recorded on-chain." });
                              }}
                              variant="default"
                              size="sm"
                              className="w-full"
                      ...
                      <MilestoneNegotiation
                        ...
                        onUpdate={() => window.dispatchEvent(new CustomEvent("escrowUpdated"))}
                      />
                    </div>
                  ))}
                </div>
                ...
                <JobManagement
                  ...
                  milestones={escrow.milestones.map((m, i) => ({
                    index: i,
                    description: (m as any).originalDescription ?? m.description ?? "",
                    amount: m.amount,
                  }))}
                  onUpdate={() => window.dispatchEvent(new CustomEvent("escrowUpdated"))}
                />
              ...
              <div className="mt-4 pt-4 border-t border-orange-200 dark:border-orange-800 space-y-3">
                <div className="flex items-start gap-2 p-3 rounded-lg bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-700">
                  ...
                  <div className="text-sm">
                    <p className="font-medium text-orange-700 dark:text-orange-400">Project deadline has passed</p>
                ...
                {escrow.isClient && onExtendDeadline && (
                  <div className="flex items-end gap-2">
                    <div className="flex-1">
                      <Label className="text-xs mb-1 block text-muted-foreground">Extend by (days)</Label>
                      <Input
                        type="number"
                        min={1}
                        max={90}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (days > 0) { onExtendDeadline(escrow.id, days); setCustomDays(""); }
                      }}
                    >
                      ...
                      <div className="space-y-2 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700">
                        ...
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button
                            ...
                            onClick={() => { onRaiseOverdueDispute(escrow.id, disputeReason); setShowDisputeForm(false); setDisputeReason(""); }}
                          >
                            Submit to Arbiters
                          </Button>
                        </div>
                ...
                {hasRating ? (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">Your Rating: {existingRating?.rating}/5</span>
                    </div>
                    {existingRating?.review && (
                      <div className="bg-muted/20 rounded-lg p-3 text-sm">{existingRating.review}</div>
                    )}
                  </div>
                ) : (
                  <Button onClick={() => setShowRatingDialog(true)} variant="outline" size="sm" className="w-full">
      ...
      {escrow.status === "completed" && escrow.beneficiary && (
        <RatingDialog
          ...
          onRatingSubmitted={async () => {
            setHasRating(true);
            try {
              const rating = await contractService.getRating(Number.parseInt(escrow.id, 10));
              if (rating) setExistingRating({ rating: rating.rating, review: rating.review });
            } catch {}
          }}
        />
      )}
